...
export function SubmissionCard({ s }: { s: Submission }) {
  const { evaluations, compare, toggleCompare, judgeWeights } = useStore();
  ...
  const aiSignal = overallSignal(s.scores);
  const composite = calculateCompositeScore(s.scores, judgeWeights);

  // When custom weights are active show: Innovation | Impact | Composite
  // When default weights: Innovation | Impact | AI Signal
  const metrics = [
    { l: "Innovation", v: s.scores.innovation },
    { l: "Impact", v: s.scores.impact },
    custom ? { l: "Composite", v: composite } : { l: "AI Signal", v: aiSignal },
  ];
    ...
    <article className="glass glass-hover flex flex-col rounded-2xl p-6">
      ...
      <div className="mt-5 grid grid-cols-3 gap-3 text-center">
        {metrics.map((m) => (
          <div
            key={m.l}
            className={`rounded-xl py-2 ${m.l === "Composite" ? "bg-primary/12 ring-1 ring-primary/25" : "bg-secondary/40"}`}
          >
      ...
      {custom && (
        <p className="mt-2 text-[10px] text-muted-foreground/70 text-center">
          AI Signal: {aiSignal} · Rankings reflect current judging criteria
        </p>
      )}
