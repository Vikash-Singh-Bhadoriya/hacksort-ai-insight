      ...
      {rows.length === 0 ? (
        <div className="glass rounded-2xl p-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Open any project and save a score to start your record.
          </p>
          <Button asChild className="mt-5">
            <Link to="/judge/submissions">Go to submissions</Link>
          </Button>
        </div>
      ) : (
            ...
            <div key={s.id} className="glass glass-hover rounded-2xl p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <Button
                  ...
                  variant="secondary"
                  onClick={() => {
                    patchEvaluation(s.id, { reviewed: !ev!.reviewed });
                    toast.success("Updated");
                  }}
                >
                  <ClipboardCheck className="mr-2 h-4 w-4" />
                  {ev!.reviewed ? "Mark unreviewed" : "Mark reviewed"}
                </Button>
                <Button
                  size="sm"
                  variant="secondary"
                  onClick={() => {
                    patchEvaluation(s.id, { shortlisted: !ev!.shortlisted });
                    toast.success("Updated");
                  }}
                >
                  <Star className="mr-2 h-4 w-4" />
                  {ev!.shortlisted ? "Unshortlist" : "Shortlist"}
                </Button>
                <Button
                  size="sm"
                  variant="secondary"
                  onClick={() => {
                    patchEvaluation(s.id, { flagged: !ev!.flagged });
                    toast.success("Updated");
                  }}
                >
                  <Flag className="mr-2 h-4 w-4" />
                  {ev!.flagged ? "Unflag" : "Flag"}
                </Button>
              </div>
            </div>
          ))}
