...
import { WeightSummaryBadge } from "@/components/CriteriaPanel";
...
const SIGNAL_ROWS: { key: keyof Scores; label: string }[] = [
...
function insight(items: Submission[]): string {
  if (items.length < 2) return "";
  const best = (k: keyof Scores) => items.reduce((a, b) => (b.scores[k] > a.scores[k] ? b : a));
  const topSignal = items.reduce((a, b) =>
    overallSignal(b.scores) > overallSignal(a.scores) ? b : a,
  );
  ...
  return [
    ...
    `${topSignal.name} carries the highest AI signal at ${overallSignal(topSignal.scores)}, driven mostly by ${topSignal.scores.impact >= topSignal.scores.innovation ? "impact" : "innovation"}.`,
    sameCat
      ? `All selected projects sit in ${items[0]?.category ?? "the same category"}, so the differentiator is execution depth rather than problem choice.`
...
function ComparePage() {
  const { submissions, compare, toggleCompare, clearCompare, judgeWeights } = useStore();
      ...
      <div className="glass mb-6 flex flex-wrap items-center gap-3 rounded-2xl p-4">
        <Select value="" onValueChange={(v) => toggleCompare(v)}>
          <SelectTrigger className="w-full sm:w-72">
            <SelectValue placeholder="Add a project to compare…" />
          </SelectTrigger>
          <SelectContent>
            {available.map((s) => (
        ...
        <div className="flex flex-wrap gap-2">
          {items.map((s) => (
            <span
              key={s.id}
              className="flex items-center gap-1.5 rounded-full border border-border/70 bg-secondary/40 px-3 py-1 text-xs"
            >
              {s.name}
      ...
      {items.length < 2 ? (
        <div className="glass rounded-2xl p-12 text-center">
          ...
          <Button asChild className="mt-5">
            <Link to="/judge/submissions">Browse submissions</Link>
          </Button>
        </div>
      ) : (
        <>
              ...
              <tbody>
                    ...
                    <tr key={r.key} className="border-b border-border/40">
                      ...
                      {items.map((s) => (
                        <td
                          ...
                          className={cn(
                            "p-4 tabular-nums",
                            s.scores[r.key] === max && "font-semibold text-primary",
                          )}
                        >
                          {s.scores[r.key]}
                        </td>
                ...
                <tr className="border-b border-border/40 bg-secondary/20">
                  <td className="p-4 text-muted-foreground text-xs">
                    AI signal <span className="text-muted-foreground/60">(fixed baseline)</span>
                  </td>
                  {items.map((s) => (
                    <td key={s.id} className="p-4 tabular-nums text-muted-foreground">
                ...
                <tr className="bg-primary/5">
                  ...
                  {items.map((s) => {
                    const vals = items.map((x) => calculateCompositeScore(x.scores, judgeWeights));
                    const maxComposite = Math.max(...vals);
                    const composite = calculateCompositeScore(s.scores, judgeWeights);
                    return (
                      <td
                        key={s.id}
                        className={cn(
                          "p-4 font-display text-lg font-semibold tabular-nums",
                          composite === maxComposite && "text-primary",
                        )}
                      >
                        {composite}
                      </td>
          ...
          <div className="mt-4 text-xs text-muted-foreground">
            Composite score reflects the criteria currently configured by the judge. Configure
            weights in the Submissions view.
          </div>
