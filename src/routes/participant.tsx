...
import { CalendarDays, CheckCircle2, Clock, FileUp, MapPin, Trophy, UserPlus, X } from "lucide-react";
...
export const Route = createFileRoute("/participant")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Join GlobalHack 2026, submit your project and track its status." },
      { property: "og:title", content: "Participant — HackSort AI" },
      { property: "og:description", content: "Register, submit a project and track review status." },
...
function Participant() {
  ...
  const register = (e: React.FormEvent) => {
    ...
    toast.success(`Welcome, ${profile.name}!`, { description: `Team ${profile.team} is registered.` });
    setTab("submit");
  };
  ...
  const submitProject = (e: React.FormEvent) => {
    ...
    toast.success("Submission received", { description: `${sub.name} filed under ${sub.category}.` });
    setTab("status");
  };
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-6">
        <div className="animate-rise">
          <Badge className="bg-primary/15 text-primary hover:bg-primary/20">Participant experience</Badge>
          ...
          <div className="mt-5 flex flex-wrap gap-4 text-sm text-muted-foreground">
            <span className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-primary" />{HACKATHON.dates}</span>
            <span className="flex items-center gap-2"><MapPin className="h-4 w-4 text-primary" />{HACKATHON.venue}</span>
            <span className="flex items-center gap-2"><Trophy className="h-4 w-4 text-primary" />{HACKATHON.prize}</span>
          </div>
        </div>
        ...
        <Tabs value={tab} onValueChange={setTab} className="mt-10">
          ...
          <TabsContent value="info" className="mt-6 grid gap-5 lg:grid-cols-2">
            ...
            <div className="glass rounded-2xl p-6">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {CATEGORIES.map((c) => (
                  <Badge key={c} variant="outline" className="border-border/70 text-muted-foreground">
                    {c}
                  </Badge>
                ))}
              </div>
              ...
              {registered ? (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <Button className="mt-4" onClick={() => setTab("submit")}>Go to submission form</Button>
                  </div>
                </div>
              ) : (
                <form className="space-y-4" onSubmit={register}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="pname" value={profile.name} onChange={(e) => setProfile({ ...profile, name: e.target.value })} required />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="pemail">Email</Label>
                    <Input id="pemail" type="email" value={profile.email} onChange={(e) => setProfile({ ...profile, email: e.target.value })} required />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="pteam">Team name</Label>
                    <Input id="pteam" value={profile.team} onChange={(e) => setProfile({ ...profile, team: e.target.value })} required />
                  </div>
                  <Button type="submit"><UserPlus className="mr-2 h-4 w-4" />Register</Button>
                </form>
              )}
            </div>
          </TabsContent>
          ...
          <TabsContent value="submit" className="mt-6">
            <form className="glass grid gap-5 rounded-2xl p-6 lg:grid-cols-2" onSubmit={submitProject}>
              <div className="space-y-2">
                <Label htmlFor="proj">Project name</Label>
                <Input id="proj" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="team">Team</Label>
                <Input id="team" value={form.team} onChange={(e) => setForm({ ...form, team: e.target.value })} required />
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">Your submission is routed to this track exactly as chosen.</p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="deck">Presentation / PPT link</Label>
                <Input id="deck" placeholder="https://…" value={form.deckUrl} onChange={(e) => setForm({ ...form, deckUrl: e.target.value })} />
              </div>

              <div className="space-y-2 lg:col-span-2">
                <Label htmlFor="prob">Problem statement</Label>
                <Textarea id="prob" rows={3} value={form.problem} onChange={(e) => setForm({ ...form, problem: e.target.value })} required />
              </div>
              <div className="space-y-2 lg:col-span-2">
                <Label htmlFor="sol">Proposed solution</Label>
                <Textarea id="sol" rows={3} value={form.solution} onChange={(e) => setForm({ ...form, solution: e.target.value })} required />
              </div>

              <div className="space-y-2 lg:col-span-2">
                <Label htmlFor="stack">Technology stack (comma separated)</Label>
                <Input id="stack" placeholder="React, Python, PyTorch" value={form.stack} onChange={(e) => setForm({ ...form, stack: e.target.value })} />
              </div>
              ...
              <div className="space-y-2 lg:col-span-2">
                ...
                {members.length ? (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {members.map((m, i) => (
                      <span key={`${m}-${i}`} className="flex items-center gap-1 rounded-full border border-border/70 bg-secondary/40 px-3 py-1 text-xs">
                        {m}
                        <button type="button" onClick={() => setMembers(members.filter((_, j) => j !== i))} aria-label={`Remove ${m}`}>
              ...
              <div className="lg:col-span-2">
                <Button type="submit"><FileUp className="mr-2 h-4 w-4" />Submit project</Button>
              </div>
            </form>
          </TabsContent>
          ...
          <TabsContent value="status" className="mt-6">
            {mySubs.length === 0 ? (
              <div className="glass rounded-2xl p-10 text-center">
                ...
                <Button className="mt-5" onClick={() => setTab("submit")}>Submit a project</Button>
              </div>
            ) : (
                ...
                {mySubs.map((s) => (
                  <div key={s.id} className="glass glass-hover rounded-2xl p-6">
                    ...
                    <div className="mt-4 flex items-center justify-between text-sm">
                      ...
                      <span className="tabular-nums text-muted-foreground">Signal {overallSignal(s.scores)}</span>
                    </div>
                  </div>
                ))}
              </div>
