...
function SubmissionsPage() {
  const { submissions, compare, clearCompare } = useStore();
  ...
  const list = useMemo(() => {
    const filtered = submissions.filter((s) => {
      const matchQ =
        !q ||
        [s.name, s.team, s.problem, s.solution, ...s.stack].join(" ").toLowerCase().includes(q.toLowerCase());
    ...
    return filtered.sort((a, b) => {
      ...
      return overallSignal(b.scores) - overallSignal(a.scores);
    });
  }, [submissions, q, cat, sort]);
    ...
    <div>
      ...
      <div className="glass mb-6 flex flex-col gap-3 rounded-2xl p-4 sm:flex-row">
        <div className="relative flex-1">
          ...
          <Input className="pl-9" placeholder="Search projects, teams, tech…" value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
        ...
        <Select value={sort} onValueChange={setSort}>
          ...
          <SelectContent>
            <SelectItem value="signal">Overall signal</SelectItem>
      ...
      {list.length === 0 ? (
        <div className="glass rounded-2xl p-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Try clearing the search term or choosing another category.</p>
        </div>
      ) : (
