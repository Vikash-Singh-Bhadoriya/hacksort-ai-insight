...
import { ArrowLeft, CheckCircle2, ExternalLink, Flag, GitCompare, Save, Star, Users } from "lucide-react";
...
export const Route = createFileRoute("/judge/submissions/$id")({
  ...
  notFoundComponent: () => (
    <div className="glass rounded-2xl p-10 text-center">
      ...
      <Button asChild className="mt-4"><Link to="/judge/submissions">Back to submissions</Link></Button>
    </div>
  ),
...
function ProjectDetail() {
  ...
  const { submissions, evaluations, saveEvaluation, patchEvaluation, toggleCompare, compare, hydrated } = useStore();
  const sub = submissions.find((s) => s.id === id);
  const existing = evaluations[id];
  const [scores, setScores] = useState<Scores>(existing?.scores ?? { innovation: 70, impact: 70, technical: 70, feasibility: 70, presentation: 70 });
  const [notes, setNotes] = useState(existing?.notes ?? "");

  if (!hydrated) return <div className="glass rounded-2xl p-8 text-sm text-muted-foreground">Loading project…</div>;
  ...
  return (
    <div className="animate-rise">
      <Link to="/judge/submissions" className="mb-6 inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">{sub.team} · submitted {new Date(sub.submittedAt).toLocaleString()}</p>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <Badge className="bg-primary/15 text-primary hover:bg-primary/20">{sub.category}</Badge>
            {cluster ? <Badge variant="outline" className="border-border/70 text-muted-foreground">{cluster.name}</Badge> : null}
            {gem ? <GemBadge /> : null}
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="secondary" onClick={() => toggleCompare(sub.id)}>
            <GitCompare className="mr-2 h-4 w-4" />{compare.includes(sub.id) ? "In comparison" : "Add to compare"}
          </Button>
          <Button asChild variant="ghost">
            <a href={sub.deckUrl || "#"} target="_blank" rel="noreferrer"><ExternalLink className="mr-2 h-4 w-4" />Deck</a>
          </Button>
        </div>
      </div>
        ...
        <div className="space-y-6 lg:col-span-2">
          <section className="glass rounded-2xl p-6">
            ...
            <div className="mt-4 space-y-4 text-sm leading-relaxed">
              ...
              <div>
                <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Solution</p>
                <p className="mt-1">{sub.solution}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Tech stack</p>
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {sub.stack.map((t) => (
                    <span key={t} className="rounded-full border border-border/60 px-2.5 py-0.5 text-xs text-muted-foreground">{t}</span>
                  ))}
                </div>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Team</p>
                <p className="mt-1 flex items-center gap-2"><Users className="h-4 w-4 text-primary" />{sub.members.join(", ")}</p>
              </div>
            </div>
          </section>
          ...
          <section className="glass rounded-2xl p-6">
            ...
            <div className="mt-4"><AiNote><p>{sub.reasoning}</p></AiNote></div>
            {gem ? (
              <div className="mt-4 rounded-xl border border-warning/30 bg-warning/8 p-4 text-sm">
                <p className="mb-2 font-medium text-warning">Hidden gem signal</p>
          ...
          <section className="glass rounded-2xl p-6">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Score independently — AI signals are context, not a verdict.</p>
            <div className="mt-6 space-y-6">
              ...
              <div>
                ...
                <Textarea rows={4} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="What convinced you, what didn't…" />
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                >
                  <CheckCircle2 className="mr-2 h-4 w-4" />{ev?.reviewed ? "Reviewed" : "Mark reviewed"}
                </Button>
                <Button
                ...
                >
                  <Star className="mr-2 h-4 w-4" />{ev?.shortlisted ? "Shortlisted" : "Shortlist"}
                </Button>
                <Button
                ...
                >
                  <Flag className="mr-2 h-4 w-4" />{ev?.flagged ? "Flagged" : "Flag"}
                </Button>
              </div>
            </div>
          </section>
        ...
        <aside className="space-y-6">
          <div className="glass rounded-2xl p-6">
            <div className="flex items-baseline justify-between">
              ...
              <span className="font-display text-3xl font-semibold text-gradient tabular-nums">{overallSignal(sub.scores)}</span>
            </div>
          ...
          {cluster ? (
            <div className="glass rounded-2xl p-6">
              <h2 className="text-lg font-semibold">Cluster context</h2>
              <p className="mt-2 text-sm text-muted-foreground">{cluster.summary}</p>
              <Button asChild variant="secondary" size="sm" className="mt-4">
                <Link to="/judge/clusters/$id" params={{ id: cluster.id }}>View cluster</Link>
              </Button>
            </div>
          ) : null}
        </aside>
