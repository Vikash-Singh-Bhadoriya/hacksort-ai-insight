...
import { byCategory, clusterSizes, gemCount, innovationDistribution, saturation, signalSpread } from "@/lib/analytics";
...
function AnalyticsPage() {
  ...
  return (
    <div>
      <PageHeader title="Analytics" lead="Distribution, saturation and progress across the judging pool." />
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <Panel title="Similarity clusters">
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={clusters} dataKey="count" nameKey="name" innerRadius={50} outerRadius={85} paddingAngle={3}>
                  {clusters.map((_, i) => (
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart>
                ...
                <XAxis type="number" dataKey="presentation" name="Presentation" domain={[40, 100]} tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} />
                <YAxis type="number" dataKey="signal" name="Signal" domain={[50, 100]} tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} />
            ...
            {sat.map((d) => (
              <div key={d.category}>
                <div className="flex justify-between text-sm">
                  ...
                  <span className="tabular-nums text-muted-foreground">{d.count} projects · {d.saturation}%</span>
                </div>
        ...
        <Panel title="Progress & gems">
          <div className="space-y-6">
            <div>
              ...
              <Progress value={Math.round((reviewed / Math.max(submissions.length, 1)) * 100)} className="mt-2" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-xl bg-secondary/40 p-4 text-center">
                <p className="font-display text-3xl font-semibold tabular-nums">{gemCount(submissions)}</p>
                <p className="mt-1 text-xs uppercase tracking-[0.12em] text-muted-foreground">Hidden gems</p>
              </div>
              <div className="rounded-xl bg-secondary/40 p-4 text-center">
                <p className="font-display text-3xl font-semibold tabular-nums">{cats.length}</p>
                <p className="mt-1 text-xs uppercase tracking-[0.12em] text-muted-foreground">Active categories</p>
              </div>
            </div>
          </div>
        </Panel>
